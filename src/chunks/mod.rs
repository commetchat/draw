...
use crate::chunks::chunk::{chunk_spawn_system, update_chunk_system};
...
fn show_chunks(
    ...
    let padding = Vec2::new(100.0, 100.0);
    ...
    while position.x < corner.x {
        while position.y < corner.y {
            if let Ok(world_pos) = camera.viewport_to_world_2d(camera_transform, position) {
                ...
                if visible_chunk_ids.contains(&id) == false {
                    visible_chunk_ids.push(id);
                }
    ...
    for id in visible_chunk_ids.iter() {
        if camera_query.2.loaded_chunks.contains(id) == false {
            events.write(ChunkEvent::Visible(id.clone()));
        }
    }

    while position.x < size.x as f32 + CHUNK_SIZE {
        while position.y < size.y as f32 + CHUNK_SIZE {
            if let Ok(world_pos) = camera.viewport_to_world_2d(camera_transform, position) {
                let pos = position_to_chunk_position(world_pos);

                let id = position_to_chunk_id(world_pos);

                let color = if visible_chunk_ids.iter().any(|e| e == &id) {
                    GREEN
                } else {
                    BLUE
                };
            ...
            position.y += CHUNK_SIZE * scale;
        }

        position.y = 0.0;
        position.x += CHUNK_SIZE * scale;
    }

    camera_query.2.loaded_chunks = visible_chunk_ids;
}
